...
export class AgentClient extends EventEmitter {
     ...
     */
    completeAuth(reqId, decision, reason) {
        const pending = this.pendingAuth.get(reqId);
        if (!pending)
            return; // 已解锁或未知请求，幂等忽略
     ...
     */
    async requestAuthorization(op) {
        const args = {
            op_type: op.type,
            description: op.description,
        };
        if (op.payload !== undefined)
            args.op_payload = JSON.stringify(op.payload);
        if (op.taskId)
            args.task_id = op.taskId;
