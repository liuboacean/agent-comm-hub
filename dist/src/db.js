...
import("./evolution.js").then(({ rebuildStrategiesFts }) => {
    if (typeof rebuildStrategiesFts === "function")
        rebuildStrategiesFts();
}).catch(() => {
    // 循环引用或函数不存在时忽略
});
// ═══════════════════════════════════════════════════════════════
 ...
 * 返回本次镜像的行数。
 */
export function enforceAuditLogCap(maxRows) {
    try {
        const row = db.prepare("SELECT COUNT(*) as cnt FROM audit_log").get();
        ...
        // 仅镜像，不删源表（WORM）
        const insertResult = db.prepare(`INSERT OR IGNORE INTO audit_log_archive
         (id, action, agent_id, target, details, ip_address, created_at, prev_hash, record_hash)
       SELECT id, action, agent_id, target, details, ip_address, created_at, prev_hash, record_hash
       FROM audit_log WHERE id IN (${placeholders})`).run(...ids);
...
export function getEnhancedDbStats() {
    ...
    try {
        ...
        logError("db_stats_archive_query_failed", err, { module: "db" });
    }
    // 最后归档时间
...
// P0-1: FTS5 索引完整性守护
// ═══════════════════════════════════════════════════════════════
export function fts5IntegrityCheck() {
    ...
    if (stratFts.cnt === 0) {
        const stratCnt = db.prepare("SELECT COUNT(*) as cnt FROM strategies WHERE status='approved'").get();
        if (stratCnt.cnt > 0) {
