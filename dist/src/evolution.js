...
export function proposeStrategy(title, content, category, proposerId, options) {
    ...
    const dupCheck = db.prepare("SELECT COUNT(*) as cnt FROM strategies WHERE title = ? AND proposer_id = ?").get(title.trim(), proposerId);
 ...
 */
export function rebuildStrategiesFts() {
    try {
        const stratCount = db.prepare(`SELECT COUNT(*) as cnt FROM strategies`).get()?.cnt ?? 0;
        let ftsCount = 0;
        try {
            ftsCount = db.prepare(`SELECT COUNT(*) as cnt FROM strategies_fts`).get()?.cnt ?? 0;
        }
        catch {
            return; // FTS 表不存在
        }
        ...
        const strategies = db.prepare(`SELECT id, title, content, category FROM strategies`).all();
        const insertFts = db.prepare(`INSERT INTO strategies_fts (rowid, title, content, category) VALUES (?, ?, ?, ?)`);
        const rebuildBatch = db.transaction((strats) => {
