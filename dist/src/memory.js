...
export function storeMemory(agentId, content, options) {
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, agentId, title, content, ftsTokens, scope, tagsStr, sourceAgentId, sourceTaskId, now, now);
        // 同步写入 FTS5 索引（P1-4/P1-5 修复：携带 memory_id 作精确关联键）
        db.prepare(`INSERT INTO memories_fts (title, content, tags, fts_tokens, memory_id) VALUES (?, ?, ?, ?, ?)`).run(title, content, tagsStr, ftsTokens, id);
        const memory = {
            id,
            agent_id: agentId,
            title,
 ...
 */
export function getMemoryStats(caller) {
    try {
...
export function rebuildFtsIndex() {
    try {
        ...
        const insertFts = db.prepare(`INSERT INTO memories_fts (title, content, tags, fts_tokens, memory_id) VALUES (?, ?, ?, ?, ?)`);
