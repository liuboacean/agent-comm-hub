...
// ═══════════════════════════════════════════════════════════════
import { isLegalAgentTransition, isLegalPipelineTransition, } from "./state-machine.js";
/**
...
export class ActivationOrchestrator {
    ...
    constructor(initialAgentStates, initialPipelineStates) {
        if (initialAgentStates)
            this.agentStates = new Map(initialAgentStates);
        if (initialPipelineStates)
            this.pipelineStates = new Map(initialPipelineStates);
    }
    /**
     ...
     */
    pausePipeline(pipelineId, operator) {
     ...
     */
    resumePipeline(pipelineId, operator) {
        const current = this.pipelineStates.get(pipelineId);
        if (!current) {
            return { success: false, error: `Pipeline not found: ${pipelineId}`, code: "PIPELINE_NOT_FOUND" };
        }
    ...
    replayFromAudit() {
        try {
            const rows = getAll(`SELECT action, target FROM audit_log WHERE action IN ('activate_agent','deactivate_agent','pause_pipeline','resume_pipeline') ORDER BY id ASC`);
            for (const row of rows) {
                if (row.action === "activate_agent")
                    this.agentStates.set(row.target, "active");
                else if (row.action === "deactivate_agent")
                    this.agentStates.set(row.target, "suspended");
                else if (row.action === "resume_pipeline")
                    this.pipelineStates.set(row.target, "active");
                else if (row.action === "pause_pipeline")
                    this.pipelineStates.set(row.target, "paused");
            }
        }
        catch {
