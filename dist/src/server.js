...
import { authMiddleware, optionalAuthMiddleware, internalMonitorAuth, requireAdminApi, createInviteCode, auditLog, rateLimiter, preAuthRateLimit, } from "./security.js";
import { startHeartbeatMonitor, stopHeartbeatMonitor, markAgentSseOnline, markAgentSseOffline, getOnlineAgentIds } from "./identity.js";
import { startDedupCleanup, stopDedupCleanup } from "./dedup.js";
import { rebuildFtsIndex } from "./memory.js";
import { logger, logError } from "./logger.js";
import { join } from "path";
import { getMetricsOutput, trackHttpRequest, incrementGauge, decrementGauge, collectHubMetrics, getTopLimited, } from "./metrics.js";
...
app.get("/health/detailed", internalMonitorAuth, (_req, res) => {
    ...
    try {
        const memMain = db.prepare("SELECT COUNT(*) as cnt FROM memories").get();
        const memFts = db.prepare("SELECT COUNT(*) as cnt FROM memories_fts").get();
        const stratFts = db.prepare("SELECT COUNT(*) as cnt FROM strategies_fts").get();
        fts5Status = memMain.cnt === memFts.cnt ? "consistent" : "drifted";
    }
    catch (err) {
        logError("health_detailed_fts5_failed", err, { module: "server" });
    }
    // 消息队列深度（24h 内未确认消息数）
    let pendingMessages = 0;
    try {
        const row = db.prepare("SELECT COUNT(*) as cnt FROM messages WHERE status IN ('unread','delivered') AND created_at > ?").get(Date.now() - 86400_000);
        pendingMessages = row?.cnt ?? 0;
    }
    catch (err) {
        logError("health_detailed_msg_pending_failed", err, { module: "server" });
    }
    res.json({
        status: "ok",
        version: HUB_VERSION,
...
app.get("/api/auth-requests", requireAdminApi, (req, res) => {
    try {
...
app.post("/api/auth-requests/:id/resolve", requireAdminApi, (req, res) => {
    ...
    try {
        ...
        const reqRow = authorizationService.resolve(req.params.id, decision, by, reason, grant_window_ms !== undefined ? Number(grant_window_ms) : undefined);
        res.json({
...
app.get("/api/status", requireAdminApi, (_req, res) => {
    ...
    try {
        const aRow = db.prepare("SELECT COUNT(*) as cnt FROM agents").get();
        dbAgentTotal = aRow?.cnt ?? 0;
        const aOnline = db.prepare("SELECT COUNT(*) as cnt FROM agents WHERE last_heartbeat > ?").get(Date.now() - 300_000);
        dbAgentOnline = aOnline?.cnt ?? 0;
        const pRow = db.prepare("SELECT COUNT(*) as cnt FROM pipelines").get();
    ...
    let last5min = 0;
    try {
        const row = db.prepare("SELECT COUNT(*) as cnt FROM messages WHERE created_at > ?").get(fiveMinAgo);
        last5min = row?.cnt ?? 0;
    }
    ...
    try {
        ...
        const agents = rows.map(r => {
            ...
            if (r.last_heartbeat == null) {
                ...
                last_seen = `${Math.floor(diff / 60_000)} 分钟前`;
            else if (diff < 86_400_000)
                last_seen = `${Math.floor(diff / 3600_000)} 小时前`;
            else
                last_seen = `${Math.floor(diff / 86_400_000)} 天前`;
            return {
                agent_id: r.agent_id,
                name: r.name,
                role: r.role,
...
function extractToolName(req) {
    try {
        ...
        logError("extract_tool_name_failed", err, { module: "mcp" });
    }
    return null;
}
// POST /mcp
...
function initServerStartTime() {
    try {
        ...
        const row = db.prepare(`SELECT value FROM server_config WHERE key='hub_first_start'`).get();
        if (row) {
