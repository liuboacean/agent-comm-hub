...
export function registerIdentityTools(server, authContext) {
    ...
    // 生成邀请码 — admin only
    // ────────────────────────────────────────────────────
    server.tool("generate_invite", "生成新 Agent 邀请码。仅 admin 可调用。邀请码默认 24 小时后过期。", {
        ...
        db.prepare("INSERT INTO auth_tokens (token_id, token_type, token_value, agent_id, role, used, created_at, expires_at) VALUES (?, 'invite_code', ?, NULL, ?, 0, ?, ?)").run(tokenId, hash, role, now, expiresAt);
