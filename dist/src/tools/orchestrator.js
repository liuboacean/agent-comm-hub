...
export let activationOrch = null;
export function setActivationOrchestrator(orch) {
    activationOrch = orch;
}
/**
...
export function registerOrchestratorTools(server, authContext) {
    ...
    // ────────────────────────────────────────────────────
    server.tool("activate_agent", "激活一个已注册的 Agent（registered/suspended → active）。已激活再激活幂等。", {
    ...
    // ────────────────────────────────────────────────────
    server.tool("deactivate_agent", "挂起一个活跃 Agent（active → suspended）。已挂起再挂起幂等。", {
        agent_id: z.string().describe("目标 Agent ID"),
    ...
    // ────────────────────────────────────────────────────
    server.tool("pause_pipeline", "暂停一个活跃 Pipeline（active → paused）。已暂停再暂停幂等。", {
    ...
    // ────────────────────────────────────────────────────
    server.tool("resume_pipeline", "恢复一个暂停的 Pipeline（paused → active）。已激活再恢复幂等。", {
        pipeline_id: z.string().describe("目标 Pipeline ID"),
