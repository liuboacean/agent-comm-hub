...
class EventLogRepo {
  ...
  appendEvent(agentId: string, eventType: string, payload: string): number {
    const info = db
      .prepare(
        `INSERT INTO event_log (agent_id, event_type, payload, delivered, created_at)
         VALUES (?, ?, ?, 0, ?)`
      )
   ...
   */
  getEventsAfter(seq: number, agentId: string, limit = 1000): StoredEvent[] {
    return db
      .prepare(
        `SELECT * FROM event_log WHERE agent_id=? AND id > ? ORDER BY id ASC LIMIT ?`
      )
      .all(agentId, seq, limit) as StoredEvent[];
  }
   ...
   */
  getUndelivered(agentId: string, limit = 1000): StoredEvent[] {
    return db
      .prepare(
        `SELECT * FROM event_log WHERE agent_id=? AND delivered=0 ORDER BY id ASC LIMIT ?`
      )
      .all(agentId, limit) as StoredEvent[];
  }
...
export const eventLogRepo: EventLogRepo = new EventLogRepo();
